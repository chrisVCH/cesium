define([
     ...
     * @param {TimeIntervalCollection} options.times TimeIntervalCollection with its data property being an object containing a uri to a Point Cloud tile and an optional transform.
     * TODO : finish adding
     * TODO : use frame state's clock?
     * TODO : clock getter setter
     */
    ...
    function getIntervalIndex(that, interval) {
        return that._times.indexOf(interval.start);
    }
    ...
    function getFrame(that, index) {
        var cache = that._cache;
        var frame = cache[index];
    ...
    TimeDynamicPointCloud.prototype.update = function(frameState) {
        ...
        var frame = getFrame(this, this._frameIndex);
        ...
        if (defined(frame)) {
            ...
            // Load the current frame, if it has not already been loaded
            var currentInterval = getCurrentInterval(this);
            if (defined(currentInterval)) {
                loadFrame(this, currentInterval);
            }
        }
        ...
        if (defined(approachingInterval)) {
            loadFrame(this, approachingInterval);
            var approachingIndex = getIntervalIndex(this, approachingInterval);
            var approachingFrame = getFrame(this, approachingIndex);
            if (defined(approachingFrame)) {
                var approachingPointCloud = approachingFrame.pointCloud;
                if (!approachingPointCloud.ready) {
                    approachingPointCloud.update(frameState);
                    commandList.length = lengthAfterUpdate; // Don't allow approaching frame to insert commands.
                }
            }
        }
